...
{ 
	...
	{ 
		...
		{ 
			if(g_temp_input_CC_value >= CHARGE_CURRENT_650_00_MA) //qus1 def 550
			{
				g_temp_input_CC_value = CHARGE_CURRENT_650_00_MA; //qus1 def 550
			}
			if(g_temp_CC_value >= CHARGE_CURRENT_650_00_MA) //qus1 def 550
			{
				g_temp_CC_value = CHARGE_CURRENT_650_00_MA; //qus1 def 550
			}
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			battery_log(BAT_LOG_CRTI,
					    "[BATTERY] select_charging_curret_bcct! type:%d\n", BMT_status.charger_type);
		} else {
			select_charging_curret();

			battery_log(BAT_LOG_CRTI, "[BATTERY] select_charging_curret! type:%d\n", BMT_status.charger_type);
		}
		battery_log(BAT_LOG_CRTI,
